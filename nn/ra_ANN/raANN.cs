...
{ 
    public partial class NeuralNetwork
    {
        ...
        private MatrixData ihWeights; // input-hidden
        private double[] hBiases;
        private double[] hOutputs;

        private MatrixData hoWeights; // hidden-output
        ...
        private MatrixData ihPrevWeightsDelta;  // for momentum with back-propagation
        private double[] hPrevBiasesDelta;
        private MatrixData hoPrevWeightsDelta;
        private double[] oPrevBiasesDelta;

        public double momentum = 0.0;
        public double weightDecay = 0.0; // 0.0001

        public MatrixData confusionMatrix = null; // warning written, printed/shown and overwritten used as a global temporary
        public int confusionMatrixCnt = 0;

        public char[] delimit = { ',', ' ', '\t' };

        public NeuralNetwork(int numInput, int numHidden, int numOutput, Random rndQ)
        {
            ...
            this.ihWeights = new MatrixData(numInput, numHidden);
            this.hBiases = new double[numHidden];
            this.hOutputs = new double[numHidden];

            this.hoWeights = new MatrixData(numHidden, numOutput);
            ...
            this.ihPrevWeightsDelta = new MatrixData(numInput, numHidden);
            this.hPrevBiasesDelta = new double[numHidden];
            this.hoPrevWeightsDelta = new MatrixData(numHidden, numOutput);
        ...
        public void train(double[][] trainData, double [][] testData, int maxEpochs, double learnRate, string logFileName, bool graph)
        {
            // train a back-prop style NN classifier using learning rate and momentum
            // weight decay reduces the magnitude of a weight value over time unless that value
            // is constantly increased
            ...
            using (StreamWriter writer = new StreamWriter(logFileName))
            {
                ...
                while (epoch < maxEpochs)
                {
                    ...
                    //if (epoch % 50 == 0) { Application.DoEvents(); } // release windows
        ...
        public double Accuracy(double[][] dataSet, string optionalOutFileName)
        {
            ...
            confusionMatrix = new MatrixData(numOutput, numOutput, typeof(int));
